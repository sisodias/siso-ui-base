  ...
  #initRenderer() {
    if (this.#config.canvas) {
      ...
      if (elem instanceof HTMLCanvasElement) {
        ...
        console.error("Ballpit Three: Missing canvas or id parameter");
      }
    } else {
      console.error("Ballpit Three: Missing canvas or id parameter");
    }
    if (!this.canvas) {
        throw new Error("Ballpit: Canvas element not found or provided.");
    }
...
const DefaultBallpitConfig = {
  count: 200,
  colors: [0x333333, 0x666666, 0x999999], // Default grayscale
...
class Z extends InstancedMesh {
  ...
  setColors(colors: number[] | Color[]) {
    if (Array.isArray(colors) && colors.length > 0) {
        const colorUtils = (function (colorsArr: (number | Color)[]) {
            let baseColors: (number | Color)[] = colorsArr;
            let colorObjects: Color[] = [];
            baseColors.forEach((col) => {
              colorObjects.push(col instanceof Color ? col : new Color(col));
            });
            return {
              setColors: (cols: (number| Color)[]) => {
                baseColors = cols;
                colorObjects = [];
                baseColors.forEach((col) => {
                  colorObjects.push(col instanceof Color ? col : new Color(col));
              ...
              getColorAt: (ratio: number, out: Color = new Color()) => {
                if (colorObjects.length === 0) return out.setHex(0xffffff); // Default to white if no colors
                if (colorObjects.length === 1) return out.copy(colorObjects[0]);

                const clamped = Math.max(0, Math.min(1, ratio));
                const scaled = clamped * (colorObjects.length - 1);
                const idx = Math.floor(scaled);
                const start = colorObjects[idx];
                if (idx >= colorObjects.length - 1) return out.copy(start);
      ...
      for (let idx = 0; idx < this.count; idx++) {
        this.setColorAt(idx, colorUtils.getColorAt(idx / (this.count -1 || 1)));
        if (idx === 0 && this.light) {
...
function createBallpit(
  ...
  const effectiveConfig = {...DefaultBallpitConfig, ...config};
  ...
  if (effectiveConfig.followCursor) {
    pointerData = createPointerData({
      domElement: canvas,
      onMove() {
        if (!pointerData || !spheres) return;
        raycaster.setFromCamera(pointerData.nPosition, threeInstance.camera);
        threeInstance.camera.getWorldDirection(plane.normal); // Align plane with camera view
        plane.constant = -threeInstance.camera.position.z; // Set plane distance to camera's Z
  ...
  function initialize(cfg: typeof DefaultBallpitConfig) {
    if (spheres) {
      threeInstance.scene.remove(spheres);
      spheres.geometry.dispose();
      (spheres.material as MeshPhysicalMaterial).dispose();
    }
...
interface BallpitProps extends React.HTMLAttributes<HTMLCanvasElement> {
  followCursor?: boolean;
  count?: number;
  gravity?: number;
  friction?: number;
...
export const Ballpit: React.FC<BallpitProps> = ({
  className,
  style,
  followCursor = DefaultBallpitConfig.followCursor,
  count = DefaultBallpitConfig.count,
  gravity = DefaultBallpitConfig.gravity,
  friction = DefaultBallpitConfig.friction,
  wallBounce = DefaultBallpitConfig.wallBounce,
  minSize = DefaultBallpitConfig.minSize,
  maxSize = DefaultBallpitConfig.maxSize,
  size0 = DefaultBallpitConfig.size0,
  colors = DefaultBallpitConfig.colors,
  ...props
}) => {
  ...
  useEffect(() => {
    ...
    spheresInstanceRef.current = createBallpit(canvas, instanceConfig);
  ...
  }, [followCursor, count, gravity, friction, wallBounce, minSize, maxSize, size0, JSON.stringify(colors), JSON.stringify(props)]);


  return <canvas className={`w-full h-full ${className || ''}`} style={style} ref={canvasRef} />;
};