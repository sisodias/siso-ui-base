...
export  function FramerMultiSlideCarousel({
  breakpoints = {
    0: { slidesToShow: 2 },
    768: { slidesToShow: 3 },
    1024: { slidesToShow: 3 },
  ...
  return (
    <div className='w-full lg:p-10 sm:p-4 p-2 max-w-5xl'>
        ...
        <div className='relative overflow-hidden rounded-lg' ref={containerRef}>
          <motion.div className='flex gap-4' style={{ x }}>
            {items.map((item) => (
              <div
                key={item.id}
                className='shrink-0 sm:h-[300px] h-[200px] rounded-lg overflow-hidden relative'
                style={{
                  width: `calc((100% - ${
                    (slidesToShow - 1) * 16
                  }px) / ${slidesToShow})`,
              ...
              >
                ...
                <div className='absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/60 to-transparent p-3'>
                  <p className='text-white sm:text-sm text-xs font-medium'>{item.title}</p>
                </div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.button
            ...
            className={`absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-lg flex items-center justify-center shadow-lg transition-transform z-10
              ${
                index === 0
          ...
          <motion.button
            ...
            className={`absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-lg flex items-center justify-center shadow-lg transition-transform z-10
              ${
                index === maxIndex
                  ? 'opacity-40 cursor-not-allowed bg-gray-300'
                  : 'bg-white hover:scale-110 hover:opacity-100 opacity-70'
