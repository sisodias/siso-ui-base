...
export const Component: React.FC = () => {
  ...
  // те же константы, что и в исходнике
  const armLimit = useMemo(() => gsap.utils.random(0, 3), []);
  const headLimit = useMemo(() => gsap.utils.random(armLimit + 1, armLimit + 3), [armLimit]);
  const angerLimit = useMemo(() => gsap.utils.random(headLimit + 1, headLimit + 3), [headLimit]);
  ...
  const SOUNDS = useMemo(() => {
    const ON = new Audio("https://assets.codepen.io/605876/switch-on.mp3");
    const OFF = new Audio("https://assets.codepen.io/605876/switch-off.mp3");
    const GROAN = new Audio("https://assets.codepen.io/605876/bear-groan.mp3");
    GROAN.playbackRate = 2;
    return { ON, OFF, GROAN };
  }, []);
  ...
  // гарантируем стартовые позиции как в исходнике
  useEffect(() => {
    gsap.set(bearRef.current, { y: "100%" });
    gsap.set(pawRef.current, { scaleX: 0 });
    gsap.set(armRef.current, { scaleX: 1 });
    gsap.set(armWrapRef.current, { x: 0 });
    gsap.set(indicatorRef.current, { x: "0%" });
    gsap.set(bgRef.current, { backgroundColor: "#aaa" });
  }, []);
  ...
  useEffect(() => {
    ...
    const grabBearTL = () => {
      let bearTranslation: "40%" | "0%" | undefined;
      if (count > armLimit && count < headLimit) bearTranslation = "40%";
      else if (count >= headLimit) bearTranslation = "0%";
      ...
      let onBearComplete: () => void = () => {};
      if (Math.random() > 0.5 && count > angerLimit) {
        onBearComplete = () => {
          SOUNDS.GROAN.play();
          if (swearRef.current) gsap.set(swearRef.current, { display: "block" });
        };
      }
      ...
      tl.add(
        count > armLimit && bearRef.current
        ...
        .to(pawRef.current, {
          ...
          scaleX: 0.8,
          onComplete: () => {
            if (swearRef.current) gsap.set(swearRef.current, { display: "none" });
          },
        })
        .to(
          bgRef.current,
    ...
    const showTimeline = () => {
      const tl = gsap.timeline({ onStart: () => SOUNDS.ON.play() });
      tl.to(bgRef.current, { duration: checkboxDuration, backgroundColor: "#2eec71" }, 0)
  ...
  return (
    <div
      className={cn("relative w-full h-[100vh] overflow-hidden flex items-center justify-center")}
      style={{ background: "#947cb0", boxSizing: "border-box" }}
    >
      {/* Вшитые стили из исходника */}
      <style>{css}</style>
...
const css = `
  ...
  right: 0;
  bottom: 0;
  background: #947cb0; /* фон страницы */
  z-index: 4;          /* выше медведя */
}
