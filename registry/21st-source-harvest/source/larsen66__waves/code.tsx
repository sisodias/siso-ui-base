...
interface WavesProps {
  lineColor?: string; backgroundColor?: string; waveSpeedX?: number;
  waveSpeedY?: number; waveAmpX?: number; waveAmpY?: number;
  xGap?: number; yGap?: number; friction?: number; tension?: number;
  maxCursorMove?: number; style?: CSSProperties; className?: string;
}

export const Waves: React.FC<WavesProps> = ({
  lineColor = "black", backgroundColor = "transparent", waveSpeedX = 0.0125,
  waveSpeedY = 0.005, waveAmpX = 32, waveAmpY = 16, xGap = 10, yGap = 32,
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          const move = noise.perlin2(
            (p.x + time * waveSpeedX) * 0.002, (p.y + time * waveSpeedY) * 0.0015
          ) * 12;
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 left-0 bg-[#160000] rounded-full"
        style={{
          width: '0.5rem',
          height: '0.5rem',
          transform: "translate3d(calc(var(--x) - 50%), calc(var(--y) - 50%), 0)",
          willChange: "transform",
          pointerEvents: 'none' // Ensure cursor div doesn't interfere with mouse events on canvas
        }}
      />
