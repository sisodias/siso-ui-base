...
/**
 * ChristmasTree
 * A self-contained SVG animation (Christmas-tree-like path with sparkles/particles).
 * Uses only free GSAP + MotionPathPlugin (no paid plugins).
 *
 * Props:
 * - width, height: overall SVG viewport CSS size (e.g., "90%", "800px")
 * - speed: global time scale (1 = normal, 0.5 = slower, 2 = faster)
 * - particleCount: how many clones to animate flying off the path
 * - colors: array of fill colors for particles (cycled)
 * - bgColor: page background (for quick embedding)
 * - showEndMessage: show the foreignObject message at end of first loop
 */
...
const FESTIVE_VIEWBOX = "0 0 800 600";
...
const ChristmasTree: React.FC<ChristmasTreeProps> = ({
  ...
  const sparkleRef = useRef<SVGCircleElement | null>(null);
  const endMessageRef = useRef<SVGForeignObjectElement | null>(null);

  // Keep IDs stable for <defs> linking
  const ids = useMemo(() => {
    ...
    return {
      ...
      grad: `grad-${rnd}`,
      dotGrad: `dotGrad-${rnd}`,
      treePathMask: `treePathMask-${rnd}`,
      treeBottomMask: `treeBottomMask-${rnd}`,
      treePotMask: `treePotMask-${rnd}`,
      glow: `glow-${rnd}`,
    };
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Load GSAP + MotionPath only in the browser
      ...
      const endMessage = endMessageRef.current!;

      // Helper: stroke "draw" animation without DrawSVGPlugin
      const drawPath = (selector: string, duration = 2, delay = 0) => {
      ...
      // Place sparkle off-screen initially
      gsap.set(sparkle, { transformOrigin: "50% 50%", y: -100, opacity: 1 });

      // Prepare particle pool by cloning particles into <g class="pContainer">
      const pool: SVGElement[] = [];
      const addToPool = (el: SVGElement) => {
      ...
      for (let i = 0; i < particleCount; i++) {
        const tpl =
          svg.querySelector<SVGElement>(
            particleSelectors[i % particleSelectors.length]
          )!.cloneNode(true) as SVGElement;
        tpl.setAttribute("fill", colors[i % colors.length]);
        addToPool(tpl);
      }

      // Helper to release one particle with pseudo-physics (no paid plugin)
      const releaseParticle = () => {
        const el = pool.shift();
        if (!el) return;

        // Start at the container position (it travels the tree path)
        const cx = gsap.getProperty(pContainer, "x") as number;
        const cy = gsap.getProperty(pContainer, "y") as number;

        gsap.set(el, { x: cx, y: cy, scale: gsap.utils.random(0.6, 2.5), opacity: 1 });

        // Random polar "impulse"
        const ang = gsap.utils.random(-Math.PI, Math.PI);
        const vel = gsap.utils.random(50, 240);
        const vx = Math.cos(ang) * vel;
        const vy0 = Math.sin(ang) * vel;
        const gravity = gsap.utils.random(60, 300);
        const dur = gsap.utils.random(0.8, 4);

        // Animate with manual gravity on onUpdate
        ...
        const tween = gsap.to(
          { t: 0 },
          {
            ...
            ease: "power1.out",
            onUpdate: function () {
              const tsec = this.targets()[0].t as number;
              const x = x0 + vx * (tsec / 2.0); // dampen
              const y = y0 + vy0 * (tsec / 2.0) + 0.5 * gravity * (tsec * tsec) * 0.0015;
              gsap.set(el, { x, y, rotation: (tsec * 360) / 2 });
            },
            onComplete: () => {
              gsap.set(el, { x: -100, y: -100, opacity: 0, scale: 0.1 });
              pool.push(el); // recycle
            },
          }
        );
        ...
        gsap.fromTo(
          ...
          { opacity: 1 },
          {
            ...
            repeat: Math.floor(gsap.utils.random(2, 10)),
            yoyo: true,
            duration: 0.07,
          }
        );
      ...
      // Animate the main sparkle & particle source along paths
      const treePath = svg.querySelector(".treePath") as SVGPathElement;
      const treeBottomPath = svg.querySelector(".treeBottomPath") as SVGPathElement;

      // One timeline that loops source along the top path, then bottom sweep
      const travelTL = gsap.timeline({
        repeat: 0,
        onUpdate: () => {
          // Periodically release a particle while "active"
          if (Math.random() < 0.7) releaseParticle();
        },
        onComplete: () => {
          if (showEndMessage) gsap.to(endMessage, { opacity: 1, duration: 0.8 });
        },
      });
      ...
      travelTL
        ...
        .to([pContainer, sparkle], {
          duration: 0.8,
          onStart: () => {
            // brief pause; particles fade a bit
            gsap.to(sparkle, { opacity: 0.6, duration: 0.3 });
          },
          x: treeBottomPath.getPointAtLength(0).x,
          y: treeBottomPath.getPointAtLength(0).y,
          ease: "power1.out",
        })
        .to([pContainer, sparkle], {
          duration: 2,
          ease: "none",
          onStart: () => gsap.to(sparkle, { opacity: 1, duration: 0.3 }),
          motionPath: { path: treeBottomPath, autoRotate: false },
        });

      // Mask “draw” animations (simulate DrawSVG)
      gsap.set(".treeBottomMask", { stroke: "#FFF" });
      drawPath(".treePathMask", 6, 0);
      drawPath(".treePotMask", 2, 0.5);
      drawPath(".treeBottomMask", 2, 0);
  ...
  }, [colors, height, particleCount, showEndMessage, speed, width, ids]);
  ...
  return (
    <div className="wrap" style={{ background: bgColor }}>
      <svg
        ...
        viewBox={FESTIVE_VIEWBOX}
      >
        <defs>
          <circle id={ids.circ} className="particle" cx="0" cy="0" r="1" />
          <polygon
            id={ids.star}
            className="particle"
          ...
          <radialGradient id={ids.grad} cx="3" cy="3" r="6" gradientUnits="userSpaceOnUse">
          ...
          <radialGradient id={ids.dotGrad} cx="0" cy="0" r="50" gradientUnits="userSpaceOnUse">
          ...
          <filter id={ids.glow} x="-150%" y="-150%" width="280%" height="280%">
        ...
        <g className="whole">
          {/* Particle container; we animate this group along the paths */}
          <g className="pContainer" ref={particlesContainerRef} />

          {/* Tree group masked to draw path */}
          <g className="tree" mask={`url(#${ids.treePathMask})`}>
            {/* (kept your three fills for the tree body) */}
            <path
          ...
          {/* Paths to follow and to "draw" */}
          <path className="treeBottomPath" fill="none" stroke="none" d="M207.5,484.1c0,0,58.5-43.1,211.1-3.1s191-16.9,191-16.9" />
          <path
            className="treePath"
            fill="none"
            stroke="none"
          ...
          <path
            ...
            mask={`url(#${ids.treeBottomMask})`}
            d="M207.5,484.06c7.05-5.11,15.14-8.66,23.34-11.63a177.13,177.13,0,0,1,25.29-6.88,263.65,263.65,0,0,1,52.22-4.49h3.28l3.28.09,6.56.19,6.55.39c2.18.13,4.37.26,6.54.48,4.35.39,8.71.74,13,1.28l6.51.75,6.49.91c17.3,2.5,34.41,6,51.36,10.19l12.62,3.2c4.18,1,8.34,2.18,12.55,3.06,8.38,2,16.82,3.63,25.29,5.13a353.5,353.5,0,0,0,51.17,5.47c17.11.32,34.36-.66,51-4.7a118.55,118.55,0,0,0,24.21-8.47,84.82,84.82,0,0,0,11.11-6.49,47.55,47.55,0,0,0,9.69-8.53,48.1,48.1,0,0,1-9,9.45,85.1,85.1,0,0,1-10.81,7.45,116.56,116.56,0,0,1-24.23,10.24,165.66,165.66,0,0,1-25.79,5.35,232.1,232.1,0,0,1-26.27,1.71c-8.77,0-17.55-.24-26.26-1.09-2.18-.2-4.37-.35-6.54-.6l-6.52-.78c-4.36-.46-8.67-1.19-13-1.82-8.64-1.37-17.22-3.09-25.74-5-4.28-.87-8.5-2-12.75-3l-12.62-3.11q-25.06-6.37-50.58-10.47a426.37,426.37,0,0,0-51.3-5.3c-8.59-.42-17.19-.29-25.78,0a240.1,240.1,0,0,0-25.68,2.24,186.57,186.57,0,0,0-25.27,5.19c-4.15,1.16-8.26,2.49-12.28,4.05-2,.79-4,1.6-6,2.52A50.82,50.82,0,0,0,207.5,484.06Z"
            fill="#cb9866"
          />
          <path
            className="treePot"
            mask={`url(#${ids.treePotMask})`}
            d="M374.32,502.55a48.15,48.15,0,0,0,1.24,14.35c1.15,4.52,3.29,8.64,6.49,11.35a18.5,18.5,0,0,0,5.51,3.14,39.06,39.06,0,0,0,6.41,1.82,65.78,65.78,0,0,0,13.68,1.12,72.9,72.9,0,0,0,13.72-1.44,44.51,44.51,0,0,0,6.46-1.85,17.75,17.75,0,0,0,5.51-3.15,25.45,25.45,0,0,0,7.24-11.17,52,52,0,0,0,1.9-6.91c.48-2.37.83-4.8,1.18-7.25a55.16,55.16,0,0,1,.64,7.42,40.11,40.11,0,0,1-.52,7.56,31.23,31.23,0,0,1-2.19,7.5,24.37,24.37,0,0,1-4.46,6.79,25.16,25.16,0,0,1-6.61,5,39.72,39.72,0,0,1-7.4,3A58.55,58.55,0,0,1,407.75,542a55,55,0,0,1-15.47-1.9,36.65,36.65,0,0,1-7.46-3,25.3,25.3,0,0,1-6.6-5,19.63,19.63,0,0,1-2.5-3.34,21.72,21.72,0,0,1-1.79-3.67,27.66,27.66,0,0,1-1.65-7.7,38.16,38.16,0,0,1,2-14.87Z"
            fill="#cb9866"
          />
          ...
          {/* Sparkle follower */}
          <circle ref={sparkleRef} className="sparkle" fill={`url(#${ids.dotGrad})`} cx="0" cy="0" r="50" />
        </g>

        {/* End message */}
        <foreignObject ref={endMessageRef} x="0" y="550" width="800" height="250">
          <span className="endMessage">
            This is available to buy at my{" "}
            <a href="https://gannon.tv/products/merry-christmas-tree" target="_blank" rel="noreferrer">
              shop
            </a>
          </span>
        </foreignObject>
      ...
      <style jsx>{`
        ...
        .mainSVG {
          width: ${width};
          height: ${height};
          visibility: hidden; /* made visible once GSAP starts */
        }
        .endMessage {
