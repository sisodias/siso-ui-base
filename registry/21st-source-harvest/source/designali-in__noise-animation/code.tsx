...
const NoiseAnimation = ({ 
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iRef.current += 0.01 * speed;  

      for (let x = 0; x < canvas.width; x += gridSize) {
        for (let y = 0; y < canvas.height; y += gridSize) {
          const point = Math.abs(noise.simplex3(
            x / scale, 
            y / scale - (iRef.current * 1.2), 
            iRef.current
          ));
          ctx.beginPath();
          ctx.fillStyle = `hsla(${(540 - (point * 540))}, ${Math.sin(point) * 100}%, 40%, ${point})`;
          ctx.arc(x, y, point * circleSize, 0, Math.PI * 2);
          ctx.fill();
          ctx.closePath();
        }
      }
  ...
  }, [speed, scale, gridSize, circleSize]);
