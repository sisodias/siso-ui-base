...
/* ── Local SubframeUtils (keep inside the same file) ───────────────────────── */
function createTwClassNames(_tokens?: string[]) {
  return (...classes: Array<string | false | null | undefined>) =>
    classes.filter(Boolean).join(" ");
}
export const SubframeUtils = {
  twClassNames: createTwClassNames([
    "text-caption",
    "text-caption-bold",
    "text-body",
    "text-body-bold",
...
/* ── Component (named + default export) ────────────────────────────────────── */
export const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  useEffect(() => {
    ...
    const effectiveDpr = Math.min(dpr ?? window.devicePixelRatio ?? 1, 2);
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (!mouseDampening || mouseDampening <= 0) {
    ...
    return () => {
      ...
      // Best-effort cleanup
      try {
        (programRef.current as unknown as { remove?: () => void })?.remove?.();
        (geometryRef.current as unknown as { remove?: () => void })?.remove?.();
