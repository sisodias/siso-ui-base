...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => (
  <textarea
    className={cn(
      "flex w-full rounded-md border-none bg-transparent px-3 py-2.5 text-base text-foreground/90 placeholder:text-foreground/50 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[44px] resize-none scrollbar-thin scrollbar-thumb-[#444444] scrollbar-track-transparent hover:scrollbar-thumb-[#555555]",
      className,
    )}
    ref={ref}
    rows={1}
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-10 rounded-full bg-foreground/10 p-2 hover:bg-foreground/15 transition-all">
...
const PromptInputTextarea: React.FC<PromptInputTextareaProps & React.ComponentProps<typeof Textarea>> = ({
  ...
  const completion = React.useMemo(() => {
    ...
    const match = autocompleteOptions.find((opt) => opt.toLowerCase().startsWith(lower) && opt.length > v.length)
    ...
    <div className="relative">
      ...
      {completion && (
        <div className="mt-1 text-xs text-foreground/50">
          Press Tab to autocomplete: <span className="text-foreground/70">{completion}</span>
        </div>
      )}
    </div>
  )
...
const SuggestionsBar: React.FC<{ items: string[] }> = ({ items }) => {
  const { value, setValue } = usePromptInput()
  if (!items?.length) return null
  const handlePick = (s: string) => {
    setValue(value ? `${value.trim()} ${s}` : s)
  }
  return (
    <div className="flex flex-wrap items-center gap-2 pb-2">
      {items.slice(0, 6).map((s, i) => (
        <button
          ...
          className="rounded-full border border-border/80 bg-background/40 px-3 py-1 text-xs text-foreground/80 hover:bg-background/60 transition-colors"
        >
          {s}
        </button>
      ))}
  ...
  return (
    <>
      <PromptInput
        ...
        onDrop={handleDrop}
      >
        {suggestions.length > 0 && !isRecording && <SuggestionsBar items={suggestions} />}
          ...
          >
            <PromptInputAction tooltip="Upload image">
              <button
                ...
                className="flex h-8 w-8 text-foreground/60 cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-foreground/10 hover:text-foreground/90"
              ...
              <button
                ...
                className={cn(
                  ...
                  showSearch
                    ...
                    : "bg-transparent border-transparent text-foreground/60 hover:text-foreground/90",
                )}
              >
              ...
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <FolderCode className={cn("w-4 h-4", showCanvas ? "text-primary" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showCanvas && (
          ...
          >
            <Button
              ...
              className={cn(
                ...
                isRecording
                  ? "bg-transparent hover:bg-foreground/10 text-red-500 hover:text-red-400"
                  : hasContent
                    ? "bg-primary text-primary-foreground hover:opacity-90"
                    : "bg-transparent hover:bg-foreground/10 text-foreground/60 hover:text-foreground/90",
              )}
              onClick={() => {
