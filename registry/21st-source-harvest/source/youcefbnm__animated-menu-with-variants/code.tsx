...
import { AnimatePresence, HTMLMotionProps, motion, MotionConfig, Variants } from 'motion/react';
...
const AnimatedMenuContext = React.createContext<AnimatedMenuContextValue | undefined>(undefined);
...
export function AnimatedMenu({ ...props }: React.HTMLAttributes<HTMLDivElement>) {
...
const buttonIconVariants = cva(
  'flex flex-col gap-1.5 justify-center items-center p-1 relative z-999 *:bg-current',
  {
    variants: {
      size: {
...
export function AnimatedMenuButtonToggleIcon({
  ...
  size = 'sm',
  ...props
}: React.HTMLAttributes<HTMLDivElement> & VariantProps<typeof buttonIconVariants>) {
  ...
  return (
    <div
      ...
      {...props}
    >
      <motion.span className="" animate={isOpen ? { rotate: 45 } : { rotate: 0 }} />
      <motion.span className="" animate={isOpen ? { rotate: -45 } : { rotate: 0 }} />
    </div>
  );
}
...
export function AnimatedMenuButton({
  ...
  return (
    <button
      className={cn(
        'appearance-none relative z-999 bg-none inline-flex gap-0.5 justify-center min-w-26 min-h-10 items-center',
...
export function AnimatedMenuList({
  ...
  return (
    <motion.div
      className={cn('fixed z-800 top-8 right-12', className)}
