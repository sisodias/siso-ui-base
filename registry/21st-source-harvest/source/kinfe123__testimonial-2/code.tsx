...
export default function FUITestimonialWithGrid() {
  return (
   <section
    ...
    className="relative py-20 w-screen sm:py-10 z-1  bg-[linear-gradient(to_right,#fff_1px,transparent_1px),linear-gradient(to_bottom,#fff_1px,transparent_1px),linear-gradient(to_bottom,#FFCC99_0%,#ffffff_30%,#fdf6e3_100%)] bg-[size:10rem_4rem,10rem_10rem,100%_100%] dark:bg-black dark:bg-none"
  >
      ...
      <Container>
        <div className="mx-auto max-w-2xl md:text-center">
          <h2 className="font-geist text-3xl tracking-tighter dark:text-gray-100 text-gray-900 sm:text-6xl">
          ...
          <p className="mt-4 text-lg tracking-tight dark:text-gray-300 text-gray-400 font-geist">
        ...
        >
          {testimonials.map((column, columnIndex) => (
            <li key={columnIndex}>
              <ul role="list" className="flex flex-col gap-y-6 sm:gap-y-8 z-10 ">
                {column.map((testimonial, testimonialIndex) => (
                  <li key={testimonialIndex}>
                    <figure className="relative rounded-2xl bg-[#FFCC99]/20 dark:bg-transparent transform-gpu [border:1px_solid_rgba(255,255,255,.1)] [box-shadow:0_-20px_80px_-20px_#8686f01f_inset] p-6 shadow-xl ">
                      <QuoteIcon className="absolute hidden left-6 top-6 fill-slate-950" />
                      <blockquote className="relative">
                        <p className="text-lg tracking-tight dark:text-gray-100 text-[#1E3A8A]">
                          {testimonial.content}
                        </p>
                      </blockquote>
                      <figcaption className="relative mt-6 flex items-center justify-between border-t border-slate-100/10 pt-6">
                        <div>
                          <div className="font-display text-base dark:text-gray-100 text-[#1E3A8A]/80">
