...
export function ArcRevealHero({
  ...
  //   t=0 → chord at y=110 (off-screen below)  → no black visible
  //   t=1 → chord at y=-30 (off-screen above)  → full-screen black
  ...
  return (
    <section
      aria-label="Hero"
      className={cn(
        "relative isolate min-h-screen w-full overflow-hidden bg-neutral-950 text-white",
        className,
      )}
    >
      ...
      <AnimatePresence>
        {showOverlay && (
          <motion.div
            ...
            className={cn(
              "absolute inset-x-0 top-0 z-30 h-screen overflow-hidden bg-white",
              introClassName,
            )}
          >
            {/* Cycled greeting */}
              ...
              <AnimatePresence mode="wait">
                {phase === "intro" && current && (
                  <motion.span
                    ...
                    className={cn(
                      "select-none px-6 text-center text-5xl font-semibold tracking-tight text-neutral-700 sm:text-6xl md:text-7xl",
                      greetingClassName,
                    )}
                  >
                    {current.text}
            ...
            <svg
              ...
              aria-hidden
            >
              <motion.path d={arcPath} fill="#0a0a0a" />
            </svg>
          </motion.div>
        )}
      </AnimatePresence>
